...
int main(int argc, char** argv)
{
	...
	num_bytes = strlen(sendbuf);
	sendbuf[num_bytes] = '\0';

	printf("get [%d] bytes data from user in total\n", num_bytes);
	fflush(stdout);

	// htonl(num_bytes);
	ret = send_Nbytes(s, (char*)&num_bytes, sizeof(int));
	if (ret != sizeof(int)){
		perror("send size error 1");
		return 0;
	}

	ret = send_Nbytes(s, sendbuf, num_bytes);
	if (ret != num_bytes){
	...
	ret = recv_Nbytes(s, recvbuf, num_bytes);
